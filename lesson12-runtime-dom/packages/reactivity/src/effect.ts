import { recordEffectScope } from "@vue/reactivity";
...
export class ReactiveEffect {
  ...
  constructor(public fn, private scheduler) {
    // 执行Effect时 让可能存在的Effect Scope去收集此Effect
...
export function effect(fn, options: any = {}) {
  ...
  // 默认让响应式的effect执行一次 执行实例的run方法就是执行 fn 触发取值操作 进行依赖收集
  _effect.run();
