...
console.log(getSequence([2, 3, 1, 5, 6, 8, 7, 9, 4])); // [0,1,2,3,4,5,6,7,8]
/**
 * 默认放arr的第一项result = [0]   对应的是[2]
 * 第一次遍历: result = [0] 取的是arr的第一项 没变化 对应的还是 [2]
 * 第二次遍历: result = [0] 取arr的第二项3, 将3对应的索引追加到result中              result = [0,1] => [2,3]
 * 第三次遍历: result = [0,1] 取arr的第三项1, 结果1比3小, 通过二分查找将2替换掉       result = [2,1] => [1,3]
 * 第四次遍历: result = [2,1] 取arr的第四项5, 结果比3大，将5对应的索引追加到result中  result = [2,1,3] => [1,3,5]
 * 第五次遍历: result = [1,3,5] 取arr的第五项6,结果比5大,将6对应的索引追加到result中  result = [2,1,3,4] => [1,3,5,6]
 * 第六次遍历: result = [0,2,3,4] 取arr的第六项8,结果比6大，将8对应的索引追加到result中      result = [2,1,3,4,5] => [1,3,5,6,8]
 * 第七次遍历: result = [2,1,3,4,5] 取arr的第七项7,结果比8小，通过二分查找将8替换掉 result = [2,1,3,4,6] => [1,3,5,6,7]
 * 第八次遍历: result = [2,1,3,4,6] 取arr的第八项9,结果比7大, 将9对应的索引追加到result中    result = [2,1,3,4,6,7] => [1,3,5,6,7,9]
 * 第九次遍历: result = [2,1,3,4,6,7] 取arr的第九项,结果比9小, 通过二分查找将5替换掉         result = [2,1,8,4,6,7] => [1,3,4,6,7,9]
 */

// 正确的答案应该是 [2,3,5,6,7,9]
